...
if exist "filelist.txt" if exist "uniquefilestocopy.txt" if not "%dynamic_shaders%" == "1" (
	...
	@REM the above does not work when using -threads, it forces it to use the new one
	@REM %shadercompilecommand% -nompi -threads %threadcount% -shaderpath "%shader_path_cd:/=\%" -allowdebug
	%shadercompilecommand% -nompi -nointercept -shaderpath "%shader_path_cd:/=\%" -allowdebug
	cd /D %shader_path_cd%
)

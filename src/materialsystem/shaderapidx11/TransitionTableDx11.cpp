...
// Singleton
//-----------------------------------------------------------------------------
CTransitionTableDx11 *g_pTransitionTable = NULL;

#ifdef DEBUG_BOARD_STATE
inline ShadowStateDx11_t& BoardState()
{
	return g_pTransitionTable->BoardState();
}
#endif

inline CTransitionTableDx11::CurrentState_t& CurrentState()
{
...
// Less functions
//-----------------------------------------------------------------------------
bool CTransitionTableDx11::ShadowStateDictLessFunc::Less( const CTransitionTableDx11::ShadowStateDictEntry_t &src1,
							  const CTransitionTableDx11::ShadowStateDictEntry_t &src2, void *pCtx )
{
	return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTableDx11::SnapshotDictLessFunc::Less( const CTransitionTableDx11::SnapshotDictEntry_t &src1, const CTransitionTableDx11::SnapshotDictEntry_t &src2, void *pCtx )
{
	return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTableDx11::UniqueSnapshotLessFunc::Less( const CTransitionTableDx11::TransitionList_t &src1, const CTransitionTableDx11::TransitionList_t &src2, void *pCtx )
{
...
//-----------------------------------------------------------------------------
CTransitionTableDx11::CTransitionTableDx11() : m_DefaultStateSnapshot(-1),
...
CTransitionTableDx11::~CTransitionTableDx11()
{
...
//-----------------------------------------------------------------------------
bool CTransitionTableDx11::Init( )
{
	return true;
}

void CTransitionTableDx11::Shutdown( )
{
	Reset();
}
...
//-----------------------------------------------------------------------------
StateSnapshot_t CTransitionTableDx11::CreateStateSnapshot( ShadowStateId_t shadowStateId, const ShadowShaderStateDx11_t& currentShaderState )
{
	...
	memcpy( &shaderState.m_ShaderState, &currentShaderState, sizeof(ShadowShaderStateDx11_t) );
...
// Creates a shadow, adding an entry into the shadow list and transition table
//-----------------------------------------------------------------------------
CTransitionTableDx11::ShadowStateId_t CTransitionTableDx11::CreateShadowState( const ShadowStateDx11_t &currentState )
{
	int newShaderState = m_ShadowStateList.AddToTail();

	// Copy our snapshot into the list
	memcpy( &m_ShadowStateList[newShaderState], &currentState, sizeof(ShadowStateDx11_t) );
	...
	CRC32_Init(	&insert.m_nChecksum );
	CRC32_ProcessBuffer( &insert.m_nChecksum, &m_ShadowStateList[newShaderState], sizeof(ShadowStateDx11_t) );
	CRC32_Final( &insert.m_nChecksum );
...
//-----------------------------------------------------------------------------
CTransitionTableDx11::ShadowStateId_t CTransitionTableDx11::FindShadowState( const ShadowStateDx11_t& currentState ) const
{
	ShadowStateDictEntry_t find;

	CRC32_Init(	&find.m_nChecksum );
	CRC32_ProcessBuffer( &find.m_nChecksum, &currentState, sizeof(ShadowStateDx11_t) );
	...
	for ( ; i < nDictCount; ++i )
	{
		...
		if (!memcmp(&m_ShadowStateList[nShadowState], &currentState, sizeof(ShadowStateDx11_t) ))
			return nShadowState;
	}
...
// Finds a snapshot, if it exists. Or creates a new one if it doesn't.
//-----------------------------------------------------------------------------
StateSnapshot_t CTransitionTableDx11::FindStateSnapshot( ShadowStateId_t id, const ShadowShaderStateDx11_t& currentState ) const
{
	...
	for ( ; i < nDictCount; ++i )
	{
		...
		if ( (id == m_SnapshotList[nShapshot].m_ShadowStateId) && 
			!memcmp(&m_SnapshotList[nShapshot].m_ShaderState, &currentState, sizeof(ShadowShaderStateDx11_t)) )
		{
			return nShapshot;
		}
	}
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::Reset()
{
...
void ApplyZWriteEnable( const ShadowStateDx11_t& shaderState, int arg )
{
	g_pShaderAPIDx11->Set
#if defined( _X360 )
...
void ApplySRGBWriteEnable( const ShadowStateDx11_t& shadowState, int stageUnused )
{
	g_pTransitionTable->ApplySRGBWriteEnable( shadowState );
}


void CTransitionTableDx11::ApplySRGBWriteEnable( const ShadowStateDx11_t& shaderState  )
{
...
void ApplyDisableFogGammaCorrection( const ShadowStateDx11_t& shadowState, int stageUnused )
{
...
void ApplyDepthTest( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplyDepthTest( state );
}

void CTransitionTableDx11::SetZEnable( D3DZBUFFERTYPE nEnable )
{
...
void CTransitionTableDx11::SetZFunc( D3DCMPFUNC nCmpFunc )
{
...
void CTransitionTableDx11::ApplyDepthTest( const ShadowStateDx11_t& state )
{
...
void ApplyAlphaTest( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplyAlphaTest( state );
}

void CTransitionTableDx11::ApplyAlphaTest( const ShadowStateDx11_t& state )
{
...
void ApplyAlphaBlend( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplyAlphaBlend( state );
}

void CTransitionTableDx11::ApplyAlphaBlend( const ShadowStateDx11_t& state )
{
...
void ApplySeparateAlphaBlend( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplySeparateAlphaBlend( state );
}

void CTransitionTableDx11::ApplySeparateAlphaBlend( const ShadowStateDx11_t& state )
{
...
//-----------------------------------------------------------------------------
void ApplyColorTextureStage( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplyColorTextureStage( state, stage );
}

void ApplyAlphaTextureStage( const ShadowStateDx11_t& state, int stage )
{
	g_pTransitionTable->ApplyAlphaTextureStage( state, stage );
}

void CTransitionTableDx11::ApplyColorTextureStage( const ShadowStateDx11_t& state, int stage )
{
...
void CTransitionTableDx11::ApplyAlphaTextureStage( const ShadowStateDx11_t& state, int stage )
{
...
void ApplyActivateFixedFunction( const ShadowStateDx11_t& state, int stage )
{
...
// Enables textures
//-----------------------------------------------------------------------------
void ApplyTextureEnable( const ShadowStateDx11_t& state, int stage )
{
...
//-----------------------------------------------------------------------------
void ApplyCullEnable( const ShadowStateDx11_t& state, int arg )
{
...
//-----------------------------------------------------------------------------
void ApplyAlphaToCoverage( const ShadowStateDx11_t& state, int arg )
{
...
//-----------------------------------------------------------------------------
void ApplyVertexBlendEnable( const ShadowStateDx11_t& state, int stage )
{
...
void ApplyFogMode( const ShadowStateDx11_t& state, int arg )
{
#ifdef RECORDING
	char buf[1024];
...
//-----------------------------------------------------------------------------
inline void CTransitionTableDx11::AddTransition( RenderStateFunc_t func )
{
...
inline void CTransitionTableDx11::AddTextureTransition( TextureStateFunc_t func, int stage )
{
	int nElem = m_TransitionOps.AddToTail();
	TransitionOp_t &op = m_TransitionOps[nElem];
...
int CTransitionTableDx11::CreateNormalTransitions( const ShadowStateDx11_t& fromState, const ShadowStateDx11_t& toState, bool bForce )
{
	int numOps = 0;
	...
	for ( i = 0; i < nStageCount; ++i )
	{
		...
		if ( !bForce && toState.m_UsingFixedFunction && fromState.m_UsingFixedFunction )
		{
			const TextureStageShadowStateDx11_t& fromTexture = fromState.m_TextureStage[i];
			const TextureStageShadowStateDx11_t& toTexture = toState.m_TextureStage[i];
...
void CTransitionTableDx11::CreateTransitionTableEntry( int to, int from )
{
	...
	const ShadowStateDx11_t& toState = m_ShadowStateList[to];
	const ShadowStateDx11_t& fromState = (from >= 0) ? m_ShadowStateList[from] : m_ShadowStateList[to];
	bool bForce = (from < 0);
...
bool CTransitionTableDx11::TestShadowState( const ShadowStateDx11_t& state, const ShadowShaderStateDx11_t &shaderState )
{
...
//-----------------------------------------------------------------------------
unsigned short CTransitionTableDx11::FindIdenticalTransitionList( unsigned short firstElem, 
					unsigned short numOps, unsigned short nFirstTest ) const
{
	VPROF("CTransitionTableDx11::FindIdenticalTransitionList");
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::TakeDefaultStateSnapshot( )
{
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::ApplyTransitionList( int snapshot, int nFirstOp, int nOpCount )
{
	VPROF("CTransitionTableDx11::ApplyTransitionList");
	// Don't bother if there's nothing to do
	if (nOpCount > 0)
	{
		// Trying to avoid function overhead here
		ShadowStateDx11_t& shadowState = m_ShadowStateList[snapshot];
...
void CTransitionTableDx11::ApplyTransition( TransitionList_t& list, int snapshot )
{
	VPROF("CTransitionTableDx11::ApplyTransition");
	...
	{
		memcpy( &m_BoardState.m_TextureStage[i], &CurrentShadowState()->m_TextureStage[i], sizeof(TextureStageShadowStateDx11_t) );
	}
	...
	const ShadowStateDx11_t &testState1 = *CurrentShadowState();
	ShadowStateDx11_t testState2 = m_BoardState;
...
//-----------------------------------------------------------------------------
StateSnapshot_t CTransitionTableDx11::TakeSnapshot( )
{
	// Do any final computation of the shadow state
	ShaderShadow()->ComputeAggregateShadowState();

	// Get the current snapshot
	const ShadowStateDx11_t& currentState = ShaderShadow()->GetShadowState();
	...
	const ShadowShaderStateDx11_t& currentShaderState = ShaderShadow()->GetShadowShaderState();
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::ApplyShaderState( const ShadowStateDx11_t &shadowState, const ShadowShaderStateDx11_t &shaderState )
{
	VPROF("CTransitionTableDx11::ApplyShaderState");
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::UseSnapshot( StateSnapshot_t snapshotId )
{
	VPROF("CTransitionTableDx11::UseSnapshot");
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::UseDefaultState( )
{
	VPROF("CTransitionTableDx11::UseDefaultState");
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::ForceDepthFuncEquals( bool bEnable )
{
...
void CTransitionTableDx11::OverrideDepthEnable( bool bEnable, bool bDepthEnable )
{
...
void CTransitionTableDx11::EnableLinearColorSpaceFrameBuffer( bool bEnable )
{
...
//-----------------------------------------------------------------------------
void CTransitionTableDx11::PerformShadowStateOverrides( )
{
	VPROF("CTransitionTableDx11::PerformShadowStateOverrides");
